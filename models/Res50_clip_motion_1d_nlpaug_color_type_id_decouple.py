...
class Res50_clip_motion_1d_nlpaug_color_type_id_decouple(nn.Module):
    def __init__(self, cfg_model):
        super(Res50_clip_motion_1d_nlpaug_color_type_id_decouple, self).__init__()
        ...
        self.fc_text_obj = nn.Sequential(nn.Linear(dim_text, dim_embedding), 
                                         nn.ReLU(inplace=True),
                                         nn.Linear(dim_embedding, dim_embedding))

        self.fc_text_motion = nn.Sequential(nn.Linear(dim_text, dim_embedding),
                                            nn.ReLU(inplace=True),
                                            nn.Linear(dim_embedding, dim_embedding))
    ...
    def forward(self, x):
        ...
        features_text_obj = self.encoder_text.encode_text(x['appearance']).to(torch.float)
        features_text_motion = self.encoder_text.encode_text(x['motion_nl']).to(torch.float)

        features_text_fusion = self.fc_text_fusion(features_text)
        features_text_obj = self.fc_text_obj(features_text_obj)
        features_text_motion = self.fc_text_motion(features_text_motion)
