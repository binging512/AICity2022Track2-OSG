...
def nlp_fix():
    ...
    anno = json.load(open('/home/zby/AICity2022Track2/nlp/train_nlp_aug_color_obj_id.json'))
    # anno = json.load(open('/data0/CityFlow_NL/test_queries.json'))
    new_anno = {}
    new_anno_path = '/home/zby/AICity2022Track2/nlp/train_nlp_aug_zh_jp_color_obj_id.json'
    # new_anno_path = '/home/zby/AICity2022Track2/nlp/test_nlp_fix_all.json'
    failure_num = 0
    while (len(list(anno.keys())) > len(list(new_anno.keys()))):
        try:
            if os.path.isfile(new_anno_path):
                new_anno = json.load(open(new_anno_path,'r'))
                saved_num = len(new_anno)
            else:
                saved_num = 0
            
            i=0
            for ii,(k,v) in enumerate(anno.items()):
                if ii<saved_num:
                    if not k == list(new_anno.keys())[ii]:
                        print(k, list(new_anno.keys())[ii])
                        print('Error')
                    i = i + 1
                    continue
                else:
                    i = i+1
                    print("{}/{} | length:{}".format(i,len(anno),len(list(new_anno.keys()))),end='\r')
                    nls = v['nl']
                    # nls_other_views = v['nl_other_views']
                    new_nls = [bd.translate('en','jp',x) for x in nls]
                    new_nls = [bd.translate('jp','en',x) for x in new_nls]
                    # new_nls_other = [bd.translate('en','zh',x) for x in nls_other_views]
                    # new_nls_other = [bd.translate('zh','en',x) for x in new_nls_other]
                    v['nl_aug_jp'] = new_nls
                    # v['nl_other_views_aug'] = new_nls_other
                    new_anno[k] = v
                    json.dump(new_anno,open(new_anno_path,'w'),indent=2)
        except:
            failure_num += 1
            print("Failure Num: {}          ".format(failure_num))
            if failure_num == 2000:
                break
            else:
                continue
